 ...
 *
 * `/boards/:boardId/cards` is the worked example; `/boards/:boardId/lists` is its analog.
 */
...
export function createApi(deps: ApiDeps = {}): Express {
  ...
  // ---- lists (analog of cards) -------------------------------------------------------------
  v1.get("/boards/:boardId/lists", (req, res) => {
    const boardId = req.params.boardId;
    if (!boardId || !boards.has(boardId)) {
      return sendProblem(res, {
        status: 404,
    ...
    const data = lists.filter((l) => l.boardId === boardId);
    res.status(200).json({ data });
  });

  v1.post("/boards/:boardId/lists", (req, res) => {
    const boardId = req.params.boardId;
    if (!boardId || !boards.has(boardId)) {
      return sendProblem(res, {
        status: 404,
    ...
    lists.push(list);
    res.status(201).location(`/v1/boards/${boardId}/lists/${list.id}`).json(list);
  });
