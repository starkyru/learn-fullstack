 ...
 * walks `User → [List]`. Read it, then do YOUR TURN: implement the analog `CardsResolver`
 * (`@Query cards` + a `@ResolveField list` that walks `Card → List`) so `cards` mirrors `users`
 * field-for-field. That `Card.list` resolver is also where Task 2's N+1 lives.
 *
 * Everything under "YOUR TURN" THROWS until you implement it. Tests import from `solution/`; point
 * them at `../src/...` to grade your own build.
 */
...
/* ─────────────────────────── YOUR TURN: mirror Users for Cards ───────────────────────────
 *
 * Give `CardsResolver` a `@Query(() => [Card]) cards()` returning `data.cards`, and a
 * `@ResolveField(() => List, { nullable: true }) list(@Parent() card)` that returns the parent list
 * via `data.listById(card.listId)` (mirror `UsersResolver`). */
...
export class CardsResolver {
  ...
  @Query(() => [Card])
  cards(): Card[] {
    throw new Error("TODO: return data.cards (mirror UsersResolver.users)");
  }

  @ResolveField(() => List, { nullable: true })
  list(@Parent() _card: Card): List | null {
    throw new Error(
      "TODO: return the parent list via data.listById(card.listId) (mirror UsersResolver.lists)",
    );
  }
}
