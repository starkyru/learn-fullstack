...
/**
 * ANALOG (solution) — a size-budget split: walk routes in order, keeping each eager while the
 * cumulative eager size stays within `budgetKb`; once the budget is spent, the rest go lazy.
 */
export function splitByBudget(routes: readonly Route[], budgetKb: number): Route[] {
  let used = 0;
  return routes.map((r) => {
    if (used + r.sizeKb <= budgetKb) {
      used += r.sizeKb;
      return { ...r, lazy: false };
    }
    return { ...r, lazy: true };
